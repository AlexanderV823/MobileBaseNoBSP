...
//Итоговая работа по модулю Мобильная разработка в 1С
	...
	Если ОтветСервера.КодСостояния <> 200 Тогда
		...
		ВызватьИсключение СтрШаблон("Error: %1", ОтветСервераСтрока);
	...
	Иначе
		...
		ОбъектДоставка = СсылкаДоставка.ПолучитьОбъект();
			...
			Если ОбъектДоставка = Неопределено Тогда
				...
				НоваяДоставка.Статус = Перечисления.СтатусыДоставок[Доставка.Статус];
		...
		Сообщить(СтрШаблон("Обмен выполнен. Создано %1, обновлено %2", КоличествоСозданных, КоличествоОбновленных));
...
//Итоговая работа по модулю Мобильная разработка в 1С
	...
	Запрос.Текст = 
			...
			|	РегистрСведений.ФотоДоставок КАК ФотоДоставок
			|ГДЕ
			|	ДоставкаИзменения.Узел = &Узел
			|	И ФотоДоставок.Доставка В (ДоставкаИзменения.Ссылка.Ссылка)";
	...
	Пока Выборка.Следующий() Цикл
		...
		ДокументДоставки.Вставить("ФотоДоставки", Строка(Выборка.Фотография));
	...
	ЗапросОтправки.УстановитьТелоИзСтроки(ТелоЗапроса);
